// 9cc-5.0.0sjis.c -  ノード作成と記号ラベル作成をnew_node関数に分割してnew_node関数をnew_binary関数、new_node_num関数をnew_num関数に関数名変更 ※ new_nodeはnew_binaryとnew_num関数でのみ使用
...
Node *unary() {
  if (consume('+'))
    return primary();
  if (consume('-'))
    return new_binary(ND_SUB, new_num(0), primary()); // 0 - primary で反転
  return primary();
}
